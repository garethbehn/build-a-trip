...
description: Verify that every third-party API call is routed through a server-side Vercel proxy with no client-exposed keys, correct request shapes (Duffel, Google Places New API, Anthropic, TTC), and graceful mock fallback when a key is absent. Use during /build and /self-test on any file under api/ or any client code that calls an external service.
---
...
## Hard rules
1. **No client-side keys.** Grep client HTML/JS for `sk-ant`, `Bearer`, `x-api-key`, `DUFFEL`, `GOOGLE_PLACES`, raw tokens. Any hit = FAIL. Keys live only in `process.env` inside `api/*`.
...
3. **Correct request shapes:**
   ...
   - **Google Places:** New API `places:searchText`, header `X-Goog-FieldMask`, body `{ textQuery }`. NOT the legacy `/textsearch/json`.
